...
if __name__ == '__main__':
    ...
    for i in range(l):
        A[0][2] = dt
        A[1][3] = dt
        xk[1][0] = math.pow(xk[0][0], 0.5)
        xk[0][0] += 1
        xk[2][0] = vx
        xk[3][0] = vy
        x_real.append(xk[0][0])
        y_real.append(xk[1][0])
        w_x = np.random.uniform(-0.1, 0.1, (1,))
        w_y = np.random.uniform(-0.1, 0.1, (1,))
        ...
        # kalman.R[0][0] = np.var(w_x)
        # kalman.R[1][1] = np.var(w_y)
        kalman.R[0][0] = 0
        kalman.R[1][1] = 0
        # print(kalman.R)
        x_w_var.append(np.var(w_x))
        y_w_var.append(np.var(w_y))
        time.sleep(0.025)
    plt.subplot(2, 2, 1)
    ...
    plt.subplot(2, 2, 3)
    plt.plot(t[:l], x_w_var)
    plt.subplot(2, 2, 4)
    plt.plot(t[:l], y_w_var)
    print(x_w_var)
